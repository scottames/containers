...
type DistroboxID = dagger.DistroboxID
...
type Distrobox = dagger.Distrobox

// DistroboxContainerWithDistroboxClonedToTmpOpts contains options for Distrobox.ContainerWithDistroboxClonedToTmp
type DistroboxContainerWithDistroboxClonedToTmpOpts = dagger.DistroboxContainerWithDistroboxClonedToTmpOpts

// DistroboxContainerWithFileFromURLOpts contains options for Distrobox.ContainerWithFileFromURL
type DistroboxContainerWithFileFromURLOpts = dagger.DistroboxContainerWithFileFromURLOpts

// DistroboxHostExecFileOpts contains options for Distrobox.HostExecFile
type DistroboxHostExecFileOpts = dagger.DistroboxHostExecFileOpts

// DistroboxHostSpawnFileOpts contains options for Distrobox.HostSpawnFile
type DistroboxHostSpawnFileOpts = dagger.DistroboxHostSpawnFileOpts
...
func (r FedoraToolbox) MarshalJSON() ([]byte, error) {
	var concrete struct {
		Registry       string
		Org            *string
		Image          string
...
func (r *FedoraToolbox) UnmarshalJSON(bs []byte) error {
	var concrete struct {
		Registry       string
		Org            *string
		Image          string
...
func invoke(ctx context.Context, parentJSON []byte, parentName string, fnName string, inputArgs map[string][]byte) (_ any, err error) {
	...
	case "FedoraToolbox":
		...
		case "Publish":
			...
			return (*FedoraToolbox).Publish(&parent, ctx, registry, imageName, additionalTags, username, secret, skipSigningConfig, skipRegistryNamespace, skipDefaultTags, latest)
		case "PublishAndSign":
			var parent FedoraToolbox
			err = json.Unmarshal(parentJSON, &parent)
			if err != nil {
			...
			return (*FedoraToolbox).PublishAndSign(&parent, ctx, registry, imageName, additionalTags, username, secret, skipSigningConfig, skipRegistryNamespace, skipDefaultTags, cosignPrivateKey, cosignPassword, dockerConfig, cosignImage, cosignUser, latest)
		case "Container":
			var parent FedoraToolbox
			err = json.Unmarshal(parentJSON, &parent)
			if err != nil {
			...
			return (*FedoraToolbox).Container(&parent, ctx)
		case "":
			var parent FedoraToolbox
			err = json.Unmarshal(parentJSON, &parent)
			if err != nil {
			...
			return New(ctx, registry, org, image, suffix, tag), nil
		default:
			return nil, fmt.Errorf("unknown function %s", fnName)
		}
	case "":
		return dag.Module().
			WithDescription("A heavily opinionated custom Fedora container image meant for use with toolbx\nor distrobox\n").
			WithObject(
				dag.TypeDef().WithObject("FedoraToolbox").
					WithFunction(
						dag.Function("Publish",
							...
							WithArg("latest", dag.TypeDef().WithKind(BooleanKind).WithOptional(true), FunctionWithArgOpts{Description: "if true the \"latest\" tag will be published", DefaultValue: JSON("false")})).
					WithFunction(
					...
					WithFunction(
						dag.Function("Container",
							...
							WithDescription("Container returns the Fedora toolbx/distrobox dagger.Container")).
					WithField("Registry", dag.TypeDef().WithKind(StringKind)).
					WithField("Org", dag.TypeDef().WithKind(StringKind).WithOptional(true)).
					WithField("Image", dag.TypeDef().WithKind(StringKind)).
					...
					WithField("Digests", dag.TypeDef().WithListOf(dag.TypeDef().WithKind(StringKind))).
					WithConstructor(
						dag.Function("New",
							dag.TypeDef().WithObject("FedoraToolbox")).
							WithArg("registry", dag.TypeDef().WithKind(StringKind).WithOptional(true), FunctionWithArgOpts{Description: "Container registry", DefaultValue: JSON("\"registry.fedoraproject.org\"")}).
							WithArg("org", dag.TypeDef().WithKind(StringKind).WithOptional(true).WithOptional(true), FunctionWithArgOpts{Description: "Container registry organization"}).
							WithArg("image", dag.TypeDef().WithKind(StringKind).WithOptional(true), FunctionWithArgOpts{Description: "Container image name", DefaultValue: JSON("\"fedora-toolbox\"")}).
							...
							WithArg("tag", dag.TypeDef().WithKind(StringKind).WithOptional(true), FunctionWithArgOpts{Description: "Tag or major release version", DefaultValue: JSON("\"40\"")}))), nil
	default:
